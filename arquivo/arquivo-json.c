...
#include "arquivo-json.h"

int inicializarArquivo(tabela *tab, int *cresceu) {
    tab->arquivo_dados = fopen("alunos-json.json", "r+b");
    if (tab->arquivo_dados == NULL) {
        tab->arquivo_dados = fopen("alunos-json.json", "a+");
...
arvore carregar_arquivo_index(tabela *tab, int *cresceu) {
    FILE *arquivo;
    size_t len;
    char nome[17], *linha = (char *)malloc(len), delim[] = "|";
    strcpy(nome, "indices-json.txt");
...
void finalizar_arquivo(tabela *tab) {
    ...
    salvar_arquivo("indices-json.txt", tab->indice);
}